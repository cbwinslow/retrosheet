...
class MatchupHistory:
    ...
    def familiarity(self) -> str:
        ...
        if self.career_pa >= 20:
            return 'familiar'
        elif self.career_pa >= 5:
            return 'some_history'
        elif self.career_pa > 0:
            return 'rare'
...
class PlatoonSplit:
    ...
    def platoon_advantage(self) -> float:
        ...
        """
        return self.vs_r_ops - self.vs_l_ops if self.player_type == 'batter' else self.vs_l_ops - self.vs_r_ops
...
class MatchupCalculator(FeatureStore):
    ...
    def load_matchup_history(self, batter_id: int, 
                              pitcher_id: int,
                              season: Optional[int] = None) -> Optional[MatchupHistory]:
    ...
    def load_platoon_split(self, player_id: int, 
                           player_type: str,
                           season: Optional[int] = None) -> Optional[PlatoonSplit]:
    ...
    def is_platoon_advantage(self, batter_id: int, pitcher_id: int,
                             season: Optional[int] = None) -> Optional[bool]:
    ...
    def compute_matchup_score(self, batter_id: int, pitcher_id: int,
                            season: int) -> float:
        ...
        if pitcher_split and batter_split and batter_split.bats:
            pitcher_vs_hand = pitcher_split.vs_r_ops if batter_split.bats == 'R' else pitcher_split.vs_l_ops
    ...
    def save_matchup_features(self, game_pk: int, at_bat_index: int,
