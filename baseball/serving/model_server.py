...
class ModelCache:
    ...
    def set(self, features: dict[str, Any], result: Any, ttl: int | None = None) -> None:
...
class ModelServer:
    ...
    def _predict_pa_outcome(self, model, features: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            'prob_on_base': sum(
                prob_dict[c] for c in ['walk', 'single', 'double', 'triple', 'home_run']
            ),
            'confidence': float(np.max(probs)),
            'timestamp': time.time(),
        }

    def _build_feature_vector(self, features: dict[str, Any], model_type: str) -> list[float]:
