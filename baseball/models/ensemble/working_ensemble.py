#!/usr/bin/env python3
"""Working Ensemble Training System for Baseball Prediction Models.

This provides a flexible, concurrent ensemble training system that:
- Trains multiple model types simultaneously (XGBoost, HistGradientBoosting)
- Uses voting and probability averaging ensembles
- Handles database connectivity and data loading
- Provides comprehensive evaluation and model persistence
"""
...
class WorkingEnsembleTrainer:
    """Working ensemble trainer with flexible model support."""
        ...
        try:
            ...
            query = f"""
                SELECT 
                    ...
                    CASE 
                        WHEN bf.zone IN (1,2,3,4,5,6,7,8,9) THEN 'strike'
                        WHEN bf.zone IN (11,12,13,14) AND bf.balls < 3 THEN 'ball_in_play'
                        ELSE 'ball'
                    END as target
            ...
            logger.info(f"Loading pitch data for seasons: {seasons}")
            ...
            categorical_cols = ['stand', 'p_throws', 'bb_type']
    ...
    def train_xgboost_model(self, X_train, y_train, X_val, y_val):
        ...
        try:
            ...
            model = xgb.XGBClassifier(
                n_estimators=20,  # Smaller for faster training
                max_depth=3,
                learning_rate=0.1,
                random_state=42,
                n_jobs=-1
    ...
    def train_hist_gb_model(self, X_train, y_train, X_val, y_val):
        ...
        try:
            ...
            model = HistGradientBoostingClassifier(
                max_iter=20,  # Smaller for faster training
                max_depth=3,
                learning_rate=0.1,
                random_state=42
            )
    ...
    def evaluate_ensemble(self, model_results, X_test, y_test):
        ...
        # Use the first successful model's label encoder for consistency
        first_encoder = None
    ...
    def train_ensemble(self, seasons=None, sample_rate=0.1):
        ...
        # Evaluate ensemble
        ensemble_results = self.evaluate_ensemble(results, X_test, y_test)
...
def main():
    ...
    try:
        results = trainer.train_ensemble(
            seasons=[2015],  # Single year for testing
