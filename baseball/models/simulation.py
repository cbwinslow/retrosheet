...
class MarkovChainSimulator(BaseSimulator):
    ...
    def __init__(
        ...
        self._transition_probs: Optional[Dict[int, List[Tuple[int, EventType, float, int]]]] = None
        self._re24_table: Optional[Dict[int, float]] = None
    ...
    def simulate_inning(
        ...
        while state.outs < 3 and not self.is_cancelled():
            ...
            # Simulate PA
            event_type, runs, new_state = self.simulate_pa(state, batter_id, pitcher_id)
...
class MonteCarloSimulator(BaseSimulator):
    ...
    def __init__(
        self,
        pa_model: Optional[PAOutcomeModel] = None,
        db_connection=None,
    ...
    def _fallback_pa(self, state: GameState) -> Tuple[EventType, int, GameState]:
        ...
        current_re = re24.get(state.base_out_state.state_id, 0.5)
...
class SimulationService:
    ...
    def get_run_status(self, run_id: UUID) -> Optional[SimulationRun]:
