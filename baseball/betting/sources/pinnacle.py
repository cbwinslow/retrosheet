...
from baseball.betting.schemas import (
    BettingMarket, MarketType, Sport, BookRegion, MarketStatus
)
...
class PinnacleSource(BaseOddsSource):
    ...
    def _authenticate(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Pinnacle auth error: {e}")
            return False
    ...
    def _make_request(self, endpoint: str, params: Optional[dict] = None) -> dict:
        ...
        if not self._auth_token:
            if not self._authenticate():
                raise RuntimeError("Failed to authenticate with Pinnacle")
    ...
    def get_live_odds(
        ...
        if not sport_id:
            raise ValueError(f"Sport {sport} not supported by Pinnacle")
    ...
    def get_game_odds(
        ...
        for mt in market_types:
            try:
                ...
                logger.error(f"Failed to fetch {mt} for game {game_id}: {e}")
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Pinnacle health check failed: {e}")
            return False
