...
class BetRecommendation(StrEnum):
...
class StrategyConstraints(BaseBettingState):
    ...
    def validate_max_bet(cls, v: float) -> float:
        if v > 10.0 and cls.risk_profile != RiskProfile.AGGRESSIVE:
            msg = 'Max bet >10% requires aggressive risk profile'
...
class BettingStrategy(BaseBettingState):
    ...
    # Identity
    strategy_id: int | None = Field(None, description='Database ID')
...
class BettingMarket(BaseBettingState):
    ...
    line: float | None = Field(None, description='Spread or total line')

    # Line movement tracking
    odds_open: int | None = Field(None, description='Opening odds')
    line_open: float | None = Field(None, description='Opening line')
...
class BetOpportunity(BaseBettingState):
    ...
    # Identity
    opportunity_id: UUID | None = Field(None)
    strategy_id: int | None = Field(None)

    # Game context
    game_id: str = Field(...)
    season: int | None = Field(None)
    game_date: date | None = Field(None)
    ...
    temperature_f: float | None = Field(None)
    wind_speed_mph: float | None = Field(None)
    home_bullpen_fatigue: float | None = Field(None, ge=0.0, le=1.0)
    away_bullpen_fatigue: float | None = Field(None, ge=0.0, le=1.0)

    # AI explanation
    ai_explanation: str | None = Field(None, description='Natural language reasoning')
...
class PlacedBet(BaseBettingState):
    ...
    line_at_placement: float | None = Field(None)

    # Outcome (filled later)
    outcome: BetOutcome = Field(default=BetOutcome.PENDING)
    profit_loss: Decimal | None = Field(None)
    settled_at: datetime | None = Field(None)

    # Settlement source
    settled_by: str | None = Field(None)
    settlement_source: str | None = Field(None, description='api, manual, feed')
...
class BettingResponse(BaseBettingState):
    ...
    strategy: BettingStrategy | None = Field(None)
    opportunity: BetOpportunity | None = Field(None)
    opportunities: list[BetOpportunity] = Field(default_factory=list)
    bet: PlacedBet | None = Field(None)

    # Error tracking
    error_code: str | None = Field(None)
    error_details: str | None = Field(None)
