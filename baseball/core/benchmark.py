...
class BenchmarkLogger:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_throughput_rows_per_sec': sum(r.throughput_rows_per_sec for r in self.results) / len(self.results) if self.results else 0,
        }
...
_default_logger: Optional[BenchmarkLogger] = None
...
def benchmark_fn(name: Optional[str] = None):
...
class QueryProfiler:
    ...
    @contextmanager
    def profile_query(self, query_name: str, sql: str, params: Optional[tuple] = None):
        ...
        try:
            ...
            logger.info(
                f"Query {query_name}: {duration:.2f}ms | "
                f"{row_count} rows | "
                f"SQL: {sql[:100]}..."
            )
