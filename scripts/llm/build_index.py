...
``data/llama_index``. The CI workflow runs this script and then performs a
sanity‑check query via the ``LangChainBaseballAgent``. To keep existing tests
passing, the script prints a placeholder confirmation line that the tests assert
on.
"""
...
def main() -> None:
    ...
    The function builds the index and writes it to ``data/llama_index``. A short
    confirmation line is printed so that the existing end‑to‑end test suite can
