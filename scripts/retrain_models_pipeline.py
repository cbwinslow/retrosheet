...
def train_model(target: str, feature_set: str = "advanced") -> bool:
    ...
    cmd = ["python3", "scripts/train_models.py", f"--target={target}", f"--feature-set={feature_set}"]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Model retraining pipeline for new data"
    )
    parser.add_argument(
        "--target",
    ...
    parser.add_argument(
        "--skip-refresh",
        action="store_true",
        help="Skip feature mart refresh"
    )
    parser.add_argument(
        "--skip-promote",
        action="store_true",
        help="Skip model promotion"
    )
