...
def fetch_schedule(season: int | None = None, days_back: int | None = None) -> bool:
    ...
    if season:
        cmd = ["python3", "scripts/fetch_mlb_schedule.py", f"--season", str(season)]
        description = f"Fetch MLB schedule for season {season}"
    else:
        cmd = ["python3", "scripts/fetch_mlb_schedule.py", f"--days-back", str(days_back or 7)]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Orchestrate current MLB season data ingestion"
    )
    ...
    parser.add_argument("--schedule-only", action="store_true", help="Only fetch schedule, don't ingest games")
    parser.add_argument("--skip-bridge", action="store_true", help="Skip bridge table population")
    parser.add_argument("--skip-xref", action="store_true", help="Skip game cross-reference completion")
    ...
    print(f"\n{'='*60}")
    print(f"CURRENT SEASON INGESTION PIPELINE")
    print(f"{'='*60}")
