    ...
    def get_active_model(self, target_id: str) -> ModelMetadata | None:
        ...
        try:
            with conn.cursor() as cur:
                ...
                if not row:
                    return None
                fs = row[6] if isinstance(row[6], dict) else json.loads(row[6]) if isinstance(row[6], str) else row[6]
                m = row[7] if isinstance(row[7], dict) else json.loads(row[7]) if isinstance(row[7], str) else row[7] if row[7] else None
