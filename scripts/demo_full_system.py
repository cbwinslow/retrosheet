...
def check_baseball_cli() -> dict[str, Any]:
    ...
    if returncode == 0:
        ...
        returncode, stdout, stderr = run_command([sys.executable, "-m", "baseball.cli", "--help"])
        if returncode == 0:
            status["available"] = True
...
def check_pipelines() -> dict[str, Any]:
    ...
    try:
        ...
        print(f"✅ PipelineService initialized")
...
def run_demo(mode: str = "quick", output: str | None = None) -> dict[str, Any]:
    ...
    if results['database'].get('connected'):
        ...
        print(f"   Database: Not connected (use --mode full to check)")
