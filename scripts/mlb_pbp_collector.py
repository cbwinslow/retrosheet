...
try:
    ...
    logging.warning(
        "pybaseball not installed — Statcast metrics will be empty. "
...
BAT_EVENTS = {
    ...
    "double_play", "triple_play", "runner_double_play",
    "error", "catcher_interf", "fan_interference", "other_out",
    "runner_out",
}
...
def _get_schedule(season: int, game_type: str = "R") -> list[dict]:
    ...
    url = (
        ...
        f"&gameType={game_type}&hydrate=team,venue"
    )
...
def _get_game_metadata_from_feed(game_pk: int) -> dict:
    ...
    try:
        ...
        pass
    return {"game_pk": game_pk, "game_date": "", "home_team": "Home",
            "away_team": "Away", "venue": ""}
...
def _load_statcast(start_dt: str, end_dt: str) -> Optional[pd.DataFrame]:
...
def _merge_statcast(pbp_df: pd.DataFrame, sc_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    wanted_sc = [
        "game_pk", "batter", "inning", "inning_topbot",
    ...
    if "pitch_number" in sc_df.columns and "at_bat_number" in sc_df.columns:
        sc = sc_df.sort_values("pitch_number").groupby(
            ["game_pk", "batter", "inning", "inning_topbot",
             "at_bat_number"], as_index=False
    ...
    sc["_sc_half"]   = sc["_sc_half"].str.lower().map(
        {"top": "top", "bot": "bottom", "bottom": "bottom"}
    )
    ...
    for col in ["launch_speed", "launch_angle", "hit_distance_sc",
                "estimated_ba_using_speedangle", "estimated_woba_using_speedangle"]:
    ...
    for gdate in sorted(by_date):
        ...
        for meta in by_date[gdate]:
            try:
                ...
                logging.error(f"  Failed game {meta['game_pk']}: {exc}")
            time.sleep(RATE_LIMIT_SLEEP)
...
COLUMN_ORDER = [
    ...
    "retro_game_id", "retro_event_id",
    "inning", "inning_half", "outs_after", "balls", "strikes",
    "batter_id", "batter_name", "pitcher_id", "pitcher_name",
    "stand", "p_throws",
    "runner_1b_id", "runner_2b_id", "runner_3b_id",
    "runner_1b", "runner_2b", "runner_3b",
    "pitch_type", "pitch_desc", "release_speed", "spin_rate",
    "plate_x", "plate_z", "zone",
    "launch_speed", "launch_angle", "hit_distance_sc",
    "estimated_ba_using_speedangle", "estimated_woba_using_speedangle",
    "event_type", "event_desc", "hit_location",
    "is_out", "rbi", "runs_scored", "pitch_sequence",
    "retro_event_cd", "retro_event_code", "retro_bat_event_fl",
]
...
if __name__ == "__main__":
    ...
    if not df.empty:
        show_cols = [
            ...
            "event_type", "pitch_sequence", "retro_event_code", "runs_scored",
        ]
