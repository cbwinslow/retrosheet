...
class event:
    ...
    def _is_explicit(self, bfrom='B'):
        return any(em[0][0] == bfrom for em in self.em)
    ...
    def _modifiers(self, modifiers):
        ...
        for mpm in modifiers:
            ...
            elif re.findall(r'^TH[H]?[1-9\)]*$', mpm):  # throw
            ...
            elif re.findall(r'^[U]?[6]?R[0-9URNHBS]*(?:\(TH\))?$', mpm):  # relay
                ...
                if self.ad[loop]:  # there is a modifier
                    ...
                    if was_out:  # re.findall('^[1-9U]+$', self.ad[loop][0]):
                        ...
                        if error:
                            ...
                            # error describer
                            error_modifier = (
                                self.am[loop][error_loop][0] if self.am[loop][error_loop] else ''
                            )
                            ...
                            ###########################   end   ##############################
                            passes = (
                                re.findall('[1-9U]+', self.ad[loop][0]) if self.ad[loop] else None
                            )
                            # append pass sequence (for location purposes)
                            self.modifiers['passes'].append(passes[0]) if passes else None
                        ...
                        if len(self.ad[loop]) > 1:
                            for loop2, describer in enumerate(self.ad[loop][1:]):
                                if loop2 != error_loop:
                                    ...
                                    if other_error:
                                        error_modifier = (
                                            self.am[loop][loop2][0] if self.am[loop][loop2] else ''
                                        )
                                        # print ('error modifier', error_modifier)
                                        if re.sub('[1-9U]', '', error_modifier) == 'TH':
                                            self.stats['fielding'].append(
                                                ['E(TH)', other_error[0][-1]],
                                            )
                                        else:
    ...
    def _play_null(self):
        self.main_play = (
            out_in_advance(self.main_play, bfrom='B') if not self._is_explicit() else self.main_play
        )  # at bat is out
    ...
    def _play_flyout(self):
        ...
        if 'FO' in mpm and not re.findall('B', mp):
            ...
            )  # B-1 except if explicily moving on advances
    ...
    def _play_pass_outs(self):
        for base_out in re.findall(r'(?:\([B123]\))', mp):
        ...
        if double_play and self.main_play['out'] + self.ad_out < 2:
            if 'FO' not in mpm:
                ...
                self.base = (
                    leave_base(self.base, bfrom='B') if not self._is_explicit() else self.base
                )
            else:
                self.base = (
                    move_base(self.base, bfrom='B', bto='1')
                    if not self._is_explicit()
                    else self.base
                )
        ...
        fielders2 = re.findall(r'[1-9]\(', mp)  # $$()$ play, with explicit outs
        ...
        fielders4 = (
            [mp[-1]] if re.findall('[1-9]$', mp) and 'GDP' in mpm else []
        )  # it was a Ground into Double Play
    ...
    def _play_error_on_out(self):
        self.main_play = (
            advance_base(self.main_play, bfrom='B') if not self._is_explicit() else self.main_play
        )  # B-1 except if explicily moving on advances
        self.base = (
            move_base(self.base, bfrom='B', bto='1') if not self._is_explicit() else self.base
        )  # B-1 except if explicily moving on advances
    ...
    def _play_cs_error(self):
        ...
        for cs in mp.split(';'):
            ...
            bfrom = PREVIOUS_BASE[cs[2]]
            self.main_play = (
                advance_base(self.main_play, bto=bto)
                if not self._is_explicit(bfrom=bfrom)
                else self.main_play
            )
            self.base = (
                move_base(self.base, bfrom=bfrom, bto=bto)
                if not self._is_explicit(bfrom=bfrom)
                else self.base
            )  # B-1 except if explicily moving on advances
    ...
    def _play_double(self):
        ...
        self.base = (
            move_base(self.base, bfrom='B', bto='2') if not self._is_explicit() else self.base
        )  # B-1 except if explicily moving on advances
        ###########################   stats   ##############################
    ...
    def _play_home_run(self):
        self.main_play = (
            advance_base(self.main_play, bto='H', bfrom='B')
            if not self._is_explicit()
            else self.main_play
        )
        self.base = (
            move_base(self.base, bfrom='B', bto='H') if not self._is_explicit() else self.base
        )  # B-1 except if explicily moving on advances
        ###########################   stats   ##############################
    ...
    def _play_po(self):
        ...
        self.main_play = (
            out_in_advance(self.main_play, bfrom=bfrom)
        ...
        passes = re.sub(r'PO[123]\(', '', mp).replace(')', '').replace('E', '')
        self.modifiers['passes'].append(passes)
    ...
    def _play_po_error(self):
        ...
        As = re.findall(r'^(?:\([1-9]+E)+', mp)  # assists to other players
        if As:
            As = As[0].replace('E', '').replace('(', '')
            for a in As:
                self.stats['fielding'].append(['A', a])
    ...
    def _play_pocs(self):
        for split in mp.split(';'):
            if split[0:2] == 'CS':
                ...
                self.main_play = (
                    out_in_advance(self.main_play, bto=bto)
                    if not self._is_explicit(bfrom)
                    else self.main_play
                )
                self.base = (
                    leave_base(self.base, bfrom=bfrom) if not self._is_explicit() else self.base
                )
                self.stats['running'].append(['CS', bfrom, bto])
            else:
                bto = split[4]
                bfrom = PREVIOUS_BASE[split[4]]
                out_in_advance(self.main_play, bto=bto) if not self._is_explicit(
                    bfrom,
                ) else self.main_play  # there are CS events together with POCS
                self.base = (
                    leave_base(self.base, bfrom=bfrom) if not self._is_explicit() else self.base
                )
                self.stats['running'].append(['CS', bfrom, bto])
            ...
            PO = re.findall(r'[1-9]\)', split)
            if PO:
                PO = PO[0].replace(')', '')
                self.stats['fielding'].append(['PO', PO[0]])

            As = re.findall(r'(?:\([^\(]+\))', split)
            if As:
                As = As[0].replace('(', '').replace(')', '')
                for a in As:
                    if a not in PO:
            ...
            passes = re.sub(r'POCS[123]\(', '', mp).replace(')', '').replace('E', '')
            self.modifiers['passes'].append(passes)
            ###########################   end   ################################
    ...
    def _play_stolen_base(self):
        for sb in mp.split(';'):
            if sb[0:2] == 'SB':
                ...
                self.main_play = (
                    advance_base(self.main_play, bto=sb[2])
                    if not self._is_explicit(bfrom)
                    else self.main_play
                )
                self.base = (
    ...
    def _play_triple(self):
        self.main_play = (
            advance_base(self.main_play, bfrom='B', bto='3')
            if not self._is_explicit()
            else self.main_play
        )
        self.base = (
            move_base(self.base, bfrom='B', bto='3') if not self._is_explicit() else self.base
        )  # B-1 except if explicily moving on advances
        ###########################   stats   ##############################
    ...
    def _main_play(self, mp, mpm):
        ...
        if mp == '99':  # error or unknown --> usually out
        ...
        elif re.findall('^[1-9]', mp) and not re.findall(r'\(', mp) and not re.findall('E', mp):
        ...
        elif re.findall(r'^[1-9](?:[1-9]*(?:\([B123]\))?)*\+?\-?$', mp):  # implicit B out or not
            for base_out in re.findall(r'(?:\([B123]\))', mp):
                expression = rf'[\-]{base_out[1]}'
                self.str.split('.')[len(self.str.split('.')) - 1]
                if (
                    not re.findall(expression, self.str.split('.')[len(self.str.split('.')) - 1])
            ...
            if (
                double_play
                and not re.findall('B', mp)
                and (self.main_play['out'] + self.ad_out) == 2
            ):
        ...
        elif re.findall(r'^CS[23H](?:\([1-9]+\))+', mp):  ##caught stealing (except errors):
            for cs in mp.split(';'):
                bto = cs[2]
                bfrom = PREVIOUS_BASE[cs[2]]
                ...
                if re.findall(
                    ...
                    self.main_play = out_in_advance(self.main_play, bto=cs[2])
                    self.base = (
                        leave_base(self.base, bfrom=bfrom)
                        if not self._is_explicit(bfrom)
                        else self.base
                    )
                ...
                passes = (
                    re.sub('CS[23H]', '', cs).replace('(', '').replace(')', '').replace('E', '')
                )
                if passes:
                    self.modifiers['passes'].append(passes)
                ###########################   end   ################################

        elif re.findall(r'^CS[23H](?:\([1-9]*E[1-9]+)+', mp):  ## caught stealing errors
            # the advance could also be explicit given the error, for more than one base.
            for cs in mp.split(';'):
                bto = cs[2]
                bfrom = PREVIOUS_BASE[cs[2]]
        ...
        elif re.findall(r'^D[0-9]*\??$', mp):  # double
            self.main_play = (
                advance_base(self.main_play, bto='2', bfrom='B')
                if not self._is_explicit()
                else self.main_play
        ...
        elif re.findall(r'^FC[1-9]?\??$', mp):  # fielders choice (also implicit B-1)
            self.main_play = (
                advance_base(self.main_play, bfrom='B')
                if not self._is_explicit()
                else self.main_play
        ...
        elif re.findall('^NP$', mp) or re.findall('^(?:OA)?(?:99)?$', mp):  # no play
            pass
        ...
        elif re.findall(r'^PO[123](?:\([1-9]+\))', mp):  # picked off of base (without error)
            bfrom = mp[2]
            bto = NEXT_BASE[mp[2]]
            ...
            for split in mp.split(';'):
                if split[0:2] == 'CS':
                    ...
                    if (
                        ...
                        or bfrom in self.move_on_error
                    ):
                        self.main_play['out'] += 1
                    else:
                        self.main_play = (
        ...
        elif re.findall(r'^POCS[23H](?:\([1-9]*E[1-9]+)', mp):  # POCS errors
            for split in mp.split(';'):
                bto = split[4]
                bfrom = PREVIOUS_BASE[split[4]]
        ...
        elif re.findall(r'^S[0-9]*\??\+?$', mp):  # single
            self.main_play = (
                advance_base(self.main_play, bfrom='B')
                if not self._is_explicit()
                else self.main_play
        ...
        elif re.findall(r'^T[0-9]*\??\+?$', mp):  # triple
            self.main_play = (
                advance_base(self.main_play, bfrom='B', bto='3')
                if not self._is_explicit()
                else self.main_play
            ...
            if 'E1' in mpm:
                ###########################   stats   ##############################
                self.main_play = (
                    advance_base(self.main_play, bfrom='B', bto='1')
                    if not self._is_explicit()
                    else self.main_play
                )
                self.base = (
    ...
    def _split_plays(self):
        ...
        # secondary play
        self.sp = re.findall(r'(?<=\+)(?:[^\.^\+^/]+)', self.str.split('.')[0])
        ...
        if not self.sp:
            ...
            self.mpm = re.findall(
                r'(?<=/)[^\+^/]+',
                self.str.split('.')[0].split('+')[0].replace('#', '').replace('+', ''),
            )
        # print ('\nmpm:\t', self.mpm)

        self.mpd = re.findall(r'(?<=\()(?:[^\)^/])+', self.str.split('.')[0].split('+')[0])
        # print ('\nmpd\t', self.mpd)

        str_spm = (
            self.str.split('.')[0].split('+', 1)[1]
            if len(self.str.split('.')[0].split('+', 1)) > 1
            else ''
        )
        self.spm = re.findall(r'(?<=/)(?:[^/^\+]+)', str_spm)
        # print ('\nspm:\t', self.spm)

        # advances:
        self.ea = (
            self.str.split('.')[len(self.str.split('.')) - 1].split(';')
            if len(self.str.split('.')) > 1
            else []
        )
        self.ea.sort(key=lambda item: (['B', '1', '2', '3'].index(item[0]), item), reverse=True)

        for advance in self.ea:
            self.em.append(re.findall(r'[1-3B][\-X][1-3H]', advance))
        for advance in self.ea:
            self.ad.append(re.findall(r'(?<=\()(?:[^\)^/]+)', advance))
        for advance in self.ea:
            describers = re.findall(r'(?<=\()(?:[^\)]+)', advance)
            if not describers:
                self.am.append([[]])
            else:
                temp = []
                for describer in describers:
                    temp.append(re.findall(r'(?<=/)[^/^\)]+', describer))
                self.am.append(temp)
