class Parser:
    ...
    def parse_file(self, year):
        ...
        infos, starting, plays, er, subs, comments, rosters, teams, metadata = (
            [] for i in range(9)
        )
        ...
        for file in zipfile.namelist():
            ...
            if file[:4] == 'TEAM':
                ...
                for _loop, row in enumerate(file_lines):
                    row = row.decode('utf-8')
                    row_type = row.rstrip('\n').split(',')[0]
        ...
        games = (
            info[~info.duplicated(subset=['game_id', 'var'], keep='last')]
            .pivot('game_id', 'var', 'value')
            .reset_index()
        )
    ...
    def save_csv(self, path=''):
        ...
        self.plays.to_csv(f'{path}plays.csv', index=False)
        self.er.to_csv(f'{path}er.csv', index=False)
        self.subs.to_csv(f'{path}subs.csv', index=False)
        self.comments.to_csv(f'{path}comments.csv', index=False)
        self.rosters.to_csv(f'{path}rosters.csv', index=False)
        self.teams.to_csv(f'{path}teams.csv', index=False)
        self.metadata.to_csv(f'{path}metadata.csv', index=False)
...
class Event:
    ...
    def parse_advance(self):
        ...
        if re.search(r'\.', self.str):  # there was an advance:
            # test using regular expressions
            # Step2: Understanding advances / outs in advances
            out_in_advance = re.findall(r'[1-3B]X[1-3H](?:\([^\)]+\))*', self.advances)
            ...
            advanced = re.findall(r'[1-3B]\-[1-3H](?:\([^\)]+\))*', self.advances)
    ...
    def _secondary_event(self, secondary_event):
        ...
        elif re.findall(r'^PO[123](?:\([1-9]+\))', secondary_event):
            self.play[secondary_event[2]] = 0
            self.play['out'] += 1

        # only the errors (allowing to stay)
        elif re.findall(r'^PO[123](?:\([1-9]*E[1-9]+)', secondary_event):
        ...
        # without errors
        elif re.findall(r'^POCS[23H](?:\([1-9]+\))', secondary_event):
        ...
        elif re.findall(r'^POCS[23H](?:\([1-9]*E[1-9]+)', secondary_event):
    ...
    def parse_event(self):
        ...
        modifiers = (
            self.str.split('.')[0].split('/')[1:]
            if len(self.str.split('.')[0].split('/')) > 1
            else []
        )
        ...
        # at least one out:
        if re.findall(r'^[1-9](?:[1-9]*(?:\([B123]\))?)*\+?\-?$', a):
            if re.findall(r'(?:\([B123]\))', a):  # double or triple play
                outs = len(re.findall(r'(?:\([B123]\))', a))
                ...
                if modifiers:
                    for modifier in modifiers:
                        if re.findall('^[B]?[PUGFL]?DP', modifier):  # double play
                            ...
                            outs = 3
                ###############################################
                if re.search(r'(?:\([B]\))', a):  # at-bat explicit out
                ...
                elif len(re.findall(r'(?:\([B123]\))', a)) != outs:  # B is implicit
                    # new addition - ad hoc
                    if (
                        len(re.findall(r'(?:\([B123]\))', a)) == 1
                        and outs == 3
                        and not re.findall(r'[B]X[1-3H](?:\([^\)]+\))*', self.advances)
                    ):
                        self.play['out'] += 1
                    ###

                    if not re.findall(r'[1-3B]X[1-3H](?:\([^\)]+\))*', self.advances) or len(
                        re.findall(r'[1-3B]X[1-3H](?:\([^\)]+\))*', self.advances),
                    ) == len(
                        re.findall(
                            r'[1-3B]X[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?',
                            self.advances,
                        ),
                    ):  # out is implicit in advances too
                        self._out_in_advance('1')
        ...
        elif re.findall(r'^CS[23H](?:\([1-9]+\))+', a):
            for cs in a.split(';'):
                self._out_in_advance(cs[2])
        ...
        ##if its a balk
            ...
            pass
            # will test for explicit

        ##double
        elif re.findall(r'^D[0-9]*\??$', a):
        ...
        elif re.findall(r'^E[1-9]\??$', a) or re.findall(r'^FC[1-9]?\??$', a):
            if not self.play['B'] == 0:
                self._advance('1')
        ...
        ## b-1 is implicit + whatever else happens in the play
            ...
            r'^I[W]?\+?(?:WP)?(?:OA)?(?:SB[23H])?(?:CS[23H](?:\([1-9]+\)))?(?:PO[1-3](?:\([1-9]+\)))?$',
            a,
        ) or re.findall('^W(?!P)', a):
            if not self.play['B'] == 0:
                self.play['1'] = 1
        ...
        ## Strikeouts. Events can happen too: SB%, CS%, OA, PO%, PB, WP and E$
            ...
            if modifiers:
                ...
                if (
                    modifiers[0] == 'NDP'
                    and re.findall(r'[B]\-[1-3H](?:\([^\)]+\))*', self.advances)
                    and re.findall(r'[1-3B]X[1-3H](?:\([^\)]+\))*', self.advances)
                ):  # no double play credited
                    self.play['out'] -= 1

                if modifiers[0] == 'TH' and re.findall(r'[B]X[1-3H](?:\([^\)]+\))*', self.advances):
                    self.play['out'] -= 1

                if modifiers[0] == 'C' and re.findall(r'[B]X[1-3H](?:\([^\)]+\))*', self.advances):
                    self.play['out'] -= 1
                ...
                if (
                    modifiers[0] == 'DP'
                    and re.findall(r'[B]X[1-3H](?:\([^\)]+\))*', self.advances)
                    and not re.findall(
                        r'[B]X[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?',
                        self.advances,
                    )
            ...
            if other_event:
                ...
                if re.findall(r'[B]\-[1-3H](?:\([^\)]+\))*', self.advances):  # Base advanced
                    self.play['out'] -= 1
                    base_advanced = re.findall(r'[B]\-[1-3H](?:\([^\)]+\))*', self.advances)[0][2]
        ...
        # without errors:
        elif re.findall(r'^PO[123](?:\([1-9]+\))', a):
            self.play[a[2]] = 0
            self.play['out'] += 1

        # only the errors (allowing to stay)
        elif re.findall(r'^PO[123](?:\([1-9]*E[1-9]+)', a):
            pass
            # will keep explicit for now, but it usually shows one base advance.

        # POCS%($$) picked off off base % (2, 3 or H) with the runner charged with a caught stealing
        # without errors
        elif re.findall(r'^POCS[23H](?:\([1-9]+\))', a):
        ...
        # only the errors (allowing advances)
        elif re.findall(r'^POCS[23H](?:\([1-9]*E[1-9]+)', a):
            pass  # will wait for explicit advances

        # single
        elif re.findall(r'^S[0-9]*\??\+?$', a):
            # print ('single')
            if not self.play['B'] == 0:
                self.play['1'] = 1
                self.play['B'] = 0
