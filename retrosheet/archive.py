...
class Event(object):
    ...
    def parse_advance(self):
        ...
        if re.search("\.", self.str):  # there was an advance:
            ...
            for error in error_out:
                if not re.findall("(?:\([1-9U/TH]+\))+", error) or re.findall(
                    "^[1-3B]X[1-3H](?:\(TH\))", error
    ...
    def _secondary_event(self, secondary_event):
        ...
        ##caught stealing errors --> calls reversed:
            ...
            "^CS[23H](?:\([1-9]*E[1-9]+)+", secondary_event
    ...
    def parse_event(self):
        ...
        if re.findall("^[1-9](?:[1-9]*(?:\([B123]\))?)*\+?\-?$", a):
            ...
            if re.findall("(?:\([B123]\))", a):  # double or triple play
                ...
                if re.search("(?:\([B]\))", a):  # at-bat explicit out
                    ...
                    # new addition - ad hoc
                    if (
                        len(re.findall("(?:\([B123]\))", a)) == 1
                        and outs == 3
                        and not re.findall("[B]X[1-3H](?:\([^\)]+\))*", self.advances)
                    ):
                        self.play["out"] += 1
                    ###

                    if not re.findall("[1-3B]X[1-3H](?:\([^\)]+\))*", self.advances) or len(
                        re.findall("[1-3B]X[1-3H](?:\([^\)]+\))*", self.advances)
                    ) == len(
                        re.findall(
                            "[1-3B]X[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?",
                            self.advances,
                    ...
                    not re.search("(?:\([B]\))", a) and len(re.findall("(?:\([B123]\))", a)) == outs
                ):
                    for out in re.findall("(?:\([B123]\))", a):
                        self._out_in_advance(str(int(out[1]) + 1))
                    self._advance("1")
        ...
        # out + error: #out is negated
            ...
            result = "out error"
            if not re.findall("[B]\-[1-3H](?:\([^\)]+\))*", self.advances) or not re.findall(
                "[B]X[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?", self.advances
            ):
                self.play["B"] = 0
                self.play["1"] = 1
        ...
        elif re.findall("^E[1-9]\??$", a) or re.findall("^FC[1-9]?\??$", a):
            result = "single"
            if not self.play["B"] == 0:
                self._advance("1")
        ...
        ## b-1 is implicit + whatever else happens in the play
            ...
            "^I[W]?\+?(?:WP)?(?:OA)?(?:SB[23H])?(?:CS[23H](?:\([1-9]+\)))?(?:PO[1-3](?:\([1-9]+\)))?$",
            a,
        ) or re.findall("^W(?!P)", a):
            result = "single"
            if not self.play["B"] == 0:
        ...
        ## Strikeouts. Events can happen too: SB%, CS%, OA, PO%, PB, WP and E$
            ...
            if modifiers:
                if modifiers[0] == "FO" and re.findall(
                    "[1-3B]X[1-3H](?:\([^\)]+\))*", self.advances
                ):
                    self.play["out"] -= 1

                if (
                    modifiers[0] == "NDP"
                    and re.findall("[B]\-[1-3H](?:\([^\)]+\))*", self.advances)
                    and re.findall("[1-3B]X[1-3H](?:\([^\)]+\))*", self.advances)
                ...
                if modifiers[0] == "C" and re.findall(
                    "[B]\-[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?", self.advances
                ):
                    self.play["out"] -= 1

                if (
                    modifiers[0] == "DP"
                    and re.findall("[B]X[1-3H](?:\([^\)]+\))*", self.advances)
                    and not re.findall(
                        "[B]X[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?",
                        self.advances,
                    )
                ...
                if re.findall("^K$", a) and modifiers[0] == "MREV" or modifiers == "UREV":
                    if re.findall(
                        "[B]\-[1-3H]", self.advances
                    ):  # base runner explicit, so no strikeout
                        self.play["out"] -= 1

            elif (
                re.findall("[B]X[1-3H](?:\([^\)]+\))*", self.advances)
                and not re.findall(
                    "[1-3B]X[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?",
                    self.advances,
                )
                or re.findall("^K$", a)
                and (
                    re.findall("[B]\-[1-3H](?:\([^\)]+\))*", self.advances)
                    or re.findall(
                        "[B]X[1-3H](?:\([^\)]+\))*(?:\([^\)]*E[^\)]+\))(?:\([^\)]+\))?",
                        self.advances,
                    )
