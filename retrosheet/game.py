...
from .helpers import game_state, pitch_count, progress
...
class parse_game(parse_row):
    ...
    def parse_start(self, start_sub='start'):
        ...
        try:
            self.current_pitcher[home_away] = (
                player_id if fielding_position == '1' else self.current_pitcher[home_away]
            )
            self.pitch_count[home_away] = (
                0 if fielding_position == '1' else self.pitch_count[home_away]
            )
        except:
            self.log.debug(
    ...
    def parse_play(self):
        ...
        if self.current_team != self.row_values[2]:
            ...
            if self.event.advances['out'] != 3:  # catching errors
                self.log.warning(
                    'INNING NO 3RD OUT:\tGame: {0}\tteam: {1}\tinning{2}\tout: {3}'.format(
                        self.game['id'],
                        self.current_team,
                        self.current_inning,
        ...
        self.event.advances = (
            ...
            if self.event.advances['out'] >= 3
            else self.event.advances
        )
        ...
        self.current_team = (
            self.row_values[2] if self.row_values[2] in ['0', '1'] else self.current_team
        )
    ...
    def parse_event(self, row_str):
        ...
        if self.row_id == 'id' or self.row_id == 'version':
            ...
            self.game[self.row_id].append(
                [self.game['id'], self.game['meta']['events'], *self.row_data],
            )
        else:
...
class parse_files(parse_games):
    ...
    def get_data(self, yearFrom=None, yearTo=None):
        ...
        progress(1, 1, f'Completed {yearFrom}-{yearTo}')
        return True
    ...
    def to_df(self):
        ...
        for _loop, game in enumerate(self.game_list):
