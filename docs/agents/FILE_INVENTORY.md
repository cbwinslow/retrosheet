...
| `sql/bridge/930_chadwick_register_bridge.sql` | **CHADWICK REGISTER BRIDGE** - Creates staging table, upsert procedure, and views for Chadwick Bureau Register data. Staging table with all 58 ID fields, `bridge.upsert_chadwick_to_player_xref()` procedure, coverage monitoring views. | Chadwick-based player cross-reference population. |
