...
FROM features.plate_appearance_examples AS examples
INNER JOIN core.games AS games
    ON examples.game_id = games.game_id
LEFT JOIN features.batter_career_prior_pa_summary AS batter_career
    ON
        examples.season = batter_career.feature_season
        AND examples.batter_id = batter_career.batter_id
LEFT JOIN features.pitcher_career_prior_pa_summary AS pitcher_career
    ON
        examples.season = pitcher_career.feature_season
        AND examples.pitcher_id = pitcher_career.pitcher_id
LEFT JOIN features.batter_pitcher_prior_matchup_summary AS matchup
    ON
        examples.season = matchup.feature_season
        AND examples.batter_id = matchup.batter_id
        AND examples.pitcher_id = matchup.pitcher_id
LEFT JOIN features.pa_context_coarse_prior_season_rates AS coarse_context
    ON
        examples.season = coarse_context.feature_season
        AND examples.batter_hand = coarse_context.batter_hand
        AND examples.pitcher_hand = coarse_context.pitcher_hand
        AND examples.outs_before = coarse_context.outs_before
        AND examples.start_bases = coarse_context.start_bases
LEFT JOIN features.park_prior_season_run_environment AS park
    ON
        examples.season = park.feature_season
        AND games.park_id = park.park_id
LEFT JOIN features.team_rolling_30_game_summary AS batting_form
    ON
        examples.game_id = batting_form.game_id
        AND examples.batting_team_id = batting_form.team_id
LEFT JOIN features.team_rolling_30_game_summary AS fielding_form
    ON
        examples.game_id = fielding_form.game_id
        AND examples.fielding_team_id = fielding_form.team_id;
...
FROM features.game_outcome_examples AS examples
INNER JOIN core.games AS games
    ON examples.game_id = games.game_id
LEFT JOIN features.batter_career_prior_pa_summary AS batter_career
    ON
...
LEFT JOIN features.park_prior_season_run_environment AS park
    ON
        examples.season = park.feature_season
        AND games.park_id = park.park_id
LEFT JOIN features.team_rolling_30_game_summary AS home_form
    ON
        examples.game_id = home_form.game_id
        AND examples.home_team_id = home_form.team_id
LEFT JOIN features.team_rolling_30_game_summary AS away_form
    ON
        examples.game_id = away_form.game_id
        AND examples.away_team_id = away_form.team_id;
