...
WITH latest AS (
    ...
    WHERE endpoint_family = 'teams'
      AND http_status = 200
    ORDER BY season, resource_key, fetched_at DESC, snapshot_id DESC
)
SELECT
    latest.season,
...
WITH latest AS (
    SELECT DISTINCT ON (resource_key)
        ...
        payload
    FROM raw_mlb.reference_snapshots
    WHERE endpoint_family = 'people'
      AND http_status = 200
    ORDER BY resource_key, fetched_at DESC, snapshot_id DESC
)
...
WITH latest AS (
    SELECT DISTINCT ON (season, resource_key)
        ...
        payload
    FROM raw_mlb.reference_snapshots
    WHERE endpoint_family = 'standings'
      AND http_status = 200
    ORDER BY season, resource_key, fetched_at DESC, snapshot_id DESC
)
SELECT
