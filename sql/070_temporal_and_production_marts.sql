...
lagged AS (
    SELECT
        team_games.*,
        lag(game_date) OVER team_order AS previous_game_date,
        lag(park_id) OVER team_order AS previous_park_id,
        lag(is_home_team) OVER team_order AS previous_is_home_team,
        lag(opponent_team_id) OVER team_order AS previous_opponent_team_id,
        row_number() OVER (
...
LEFT JOIN core.players players
  ON players.retrosheet_player_id = plate_appearances.batter_id
...
FROM core.plate_appearances plate_appearances
LEFT JOIN core.players players
  ON players.retrosheet_player_id = plate_appearances.pitcher_id
...
LEFT JOIN features.team_game_context home_context
  ON home_context.game_id = advanced.game_id
 AND home_context.team_id = advanced.home_team_id
LEFT JOIN features.team_game_context away_context
  ON away_context.game_id = advanced.game_id
 AND away_context.team_id = advanced.away_team_id;
...
LEFT JOIN features.team_game_context batting_context
  ON batting_context.game_id = advanced.game_id
 AND batting_context.team_id = advanced.batting_team_id
LEFT JOIN features.team_game_context fielding_context
  ON fielding_context.game_id = advanced.game_id
 AND fielding_context.team_id = advanced.fielding_team_id;

CREATE OR REPLACE VIEW features.temporal_production_validation_summary AS
SELECT 'features.team_game_context' AS object_name, count(*) AS row_count
FROM features.team_game_context
UNION ALL
SELECT 'features.player_production_season', count(*)
FROM features.player_production_season
UNION ALL
SELECT 'features.pitcher_production_season', count(*)
FROM features.pitcher_production_season
UNION ALL
SELECT 'features.game_outcome_temporal_examples_2025', count(*)
FROM features.game_outcome_temporal_examples
WHERE season = 2025
UNION ALL
SELECT 'features.plate_appearance_temporal_examples_2025', count(*)
